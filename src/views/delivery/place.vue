...
<script>
  ...
  import { queryList,expresser,change_status } from '@/api/deliveryPlace'
  export default {
  name: 'collect',
  data() {
      return {
  ...
  methods: {
      ...
      async onOpenDetailDrawer(index) {
          try {
              ...
              await change_status(this.view_Data[index].id,this.employee.name,this.employee.id,this.employee.phone); // 等待 change_status 执行完
