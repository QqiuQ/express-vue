...
import { confirmDispatchTask, dispatchTasks } from '@/api/courierTask'
...
export default {
  ...
  methods: {
    dispatchToStation(row) {
      ...
      this.$confirm('是否妥投？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // yes
        return confirmDispatchTask(this.deliveryId, this.employee.id, stationId)
      }).then(res => {
        this.$message.success('妥投成功')
        this.stationDialogVisible = false
        this.query()
      }).catch(_ => {
        this.$message.error('妥投失败')
      })
    },
    ...
    query() {
      ...
      dispatchTasks(this.employee.id).then(res => {
